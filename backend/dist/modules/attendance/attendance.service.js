"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.listAttendance = listAttendance;
exports.processQrScan = processQrScan;
exports.createManualAttendance = createManualAttendance;
exports.updateAttendance = updateAttendance;
exports.calcTotalHours = calcTotalHours;
const attendance_model_1 = __importDefault(require("./attendance.model"));
const employee_model_1 = __importDefault(require("../employees/employee.model"));
const errorHandler_1 = require("../../middleware/errorHandler");
const utils_1 = require("../../shared/utils");
async function listAttendance(query, departmentFilter) {
    const { page, limit, skip } = (0, utils_1.getPagination)(query);
    const filter = {};
    if (query.employeeId)
        filter.employee = query.employeeId;
    if (departmentFilter) {
        const empIds = await employee_model_1.default.find({ department: departmentFilter, isActive: true })
            .distinct('_id');
        filter.employee = { $in: empIds };
    }
    if (query.startDate || query.endDate) {
        filter.date = {};
        if (query.startDate)
            filter.date.$gte = new Date(query.startDate);
        if (query.endDate)
            filter.date.$lte = new Date(query.endDate);
    }
    const [data, total] = await Promise.all([
        attendance_model_1.default.find(filter)
            .populate('employee', 'firstName lastName department')
            .sort({ date: -1 })
            .skip(skip)
            .limit(limit)
            .lean(),
        attendance_model_1.default.countDocuments(filter),
    ]);
    return {
        success: true,
        data: data,
        meta: { total, page, limit, totalPages: Math.ceil(total / limit) },
    };
}
/**
 * QR-based check-in / check-out.
 * First scan = check-in, second scan = check-out.
 */
async function processQrScan(email) {
    const employee = await employee_model_1.default.findOne({ email, isActive: true });
    if (!employee)
        throw new errorHandler_1.AppError('Employee not found.', 404);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const existing = await attendance_model_1.default.findOne({ employee: employee._id, date: today });
    if (!existing) {
        await attendance_model_1.default.create({
            employee: employee._id,
            date: today,
            checkIn: new Date(),
        });
        return 'Checked in successfully.';
    }
    if (!existing.checkOut) {
        existing.checkOut = new Date();
        await existing.save();
        return 'Checked out successfully.';
    }
    throw new errorHandler_1.AppError('Already checked in and out for today.', 400);
}
async function createManualAttendance(input) {
    const employee = await employee_model_1.default.findById(input.employeeId);
    if (!employee)
        throw new errorHandler_1.AppError('Employee not found.', 404);
    const date = input.date ? new Date(input.date) : new Date();
    date.setHours(0, 0, 0, 0);
    const record = await attendance_model_1.default.findOneAndUpdate({ employee: input.employeeId, date }, {
        ...(input.checkIn && { checkIn: new Date(input.checkIn) }),
        ...(input.checkOut && { checkOut: new Date(input.checkOut) }),
    }, { upsert: true, new: true, runValidators: true });
    return record;
}
async function updateAttendance(id, input) {
    const record = await attendance_model_1.default.findByIdAndUpdate(id, {
        ...(input.checkIn && { checkIn: new Date(input.checkIn) }),
        ...(input.checkOut && { checkOut: new Date(input.checkOut) }),
    }, { new: true, runValidators: true });
    if (!record)
        throw new errorHandler_1.AppError('Attendance record not found.', 404);
    return record;
}
/** Returns total hours worked in a given month/year for payroll & performance. */
async function calcTotalHours(employeeId, month, year) {
    const start = new Date(year, month - 1, 1);
    const end = new Date(year, month, 0, 23, 59, 59);
    const records = await attendance_model_1.default.find({
        employee: employeeId,
        date: { $gte: start, $lte: end },
        checkIn: { $ne: null },
        checkOut: { $ne: null },
    }).lean();
    const totalMs = records.reduce((sum, r) => {
        const ms = new Date(r.checkOut).getTime() - new Date(r.checkIn).getTime();
        return sum + (ms > 0 ? ms : 0);
    }, 0);
    return parseFloat((totalMs / 3_600_000).toFixed(2));
}
//# sourceMappingURL=attendance.service.js.map